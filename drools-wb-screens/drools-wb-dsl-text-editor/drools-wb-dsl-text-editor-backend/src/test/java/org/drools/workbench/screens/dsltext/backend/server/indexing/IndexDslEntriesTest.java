...
public class IndexDslEntriesTest extends BaseIndexingTest<DSLResourceTypeDefinition> {
    ...
    public void testIndexDslEntries() throws IOException, InterruptedException {
        ...
        {
            final Query query = new SingleTermQueryBuilder(new ValueReferenceIndexTerm("org.drools.workbench.screens.dsltext.backend.server.indexing.classes.Mortgage",
                                                                                       ResourceType.JAVA))
                    .build();
            searchFor(query,
                      1,
